...
def embed_images(request: EmbedImagesRequest):
    ...
    Called asynchronously by Java backend after image upload.
    This is a background operation - Java doesn't wait for completion.

    Args:
        request: User ID, folder ID, and list of images to embed
...
def embed_text(request: dict):
    ...
    Called by Java search service for semantic search.
    Returns raw (unnormalized) embedding - caller is responsible for normalization.

    Args:
        request: {"text": "query string"}
    ...
    try:
        ...
        # Return raw embedding (unnormalized) - Java service will normalize
...
def embed_image(request: dict):
    ...
    Called by Java search service for image indexing.
    Returns raw (unnormalized) embedding - caller is responsible for normalization.
    ...
    try:
        ...
        embedding = embedding_service.embed_image_file(image_path)
